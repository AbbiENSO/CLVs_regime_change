...
class LyapunovsEstimator(object):
    ...
    def compute_lyapunovs(self, t0, tw, t, dt, mdt, ic=None, write_steps=1, n_vec=None, forward=False, adjoint=True,
                          inverse=False):
...
if __name__ == "__main__":
    ...
    D_v = np.zeros(nr)
    step=1
    # first =  round(step + r , 2)    
    array_list = []  
    path_base = "./"
#    exp_name = "lyapunov_result_r_{k}".format(k=)

    
    
    # if not os.path.isdir(path_base+"/"+exp_name):
    #     os.mkdir(path_base+"/"+exp_name)
    #     os.mkdir(path_base+"/"+exp_name+"/figure")
    
    for i in tqdm(range(len(rhov))):
                    

        @njit
        def fL63(t, x):
            xx = sigma * (x[1] - x[0])
            yy = rhov[i] * x[0] - x[1] - x[0] * x[2]
            zz = x[0] * x[1] - bb * x[2]
...
#%% Compute Lyapunovs    
    ...
    #compute_lyapunovs(self, t0, tw, t, dt, mdt, ic=None, write_steps=1, n_vec=None, forward=False, adjoint=False,
     #                     inverse=False):
     # Parameters
     # ----------
     # t0: float
     ...
     # ic: None or ~numpy.ndarray(float), optional
        ...
        # bexp_df = pd.DataFrame(bexp.T, columns = ['x','y','z'])
        # betraj_df =  pd.DataFrame(btraj.T, columns = ['x','y','y'])
        
        # bexp_df.to_csv("bexp.csv")
        # betraj_df.to_csv("btraj.csv")
        ...
        # fexp_df = pd.DataFrame(fexp.T, columns = ['x','y','z'])
        # fetraj_df =  pd.DataFrame(ftraj.T, columns = ['x','y','y'])
        # fexp_df.to_csv("fexp.csv")
        # fetraj_df.to_csv("ftraj.csv")
        ...
        clvint.compute_clvs(0., 10000., 300000., 590000., 0.01, 0.01, ic, write_steps=1, adjoint = True) #, n_vec=2)
        ctl, ctraj, cexp, cvec = clvint.get_clvs()
    
        cexp_df = pd.DataFrame(cexp.T, columns = ['x','y','z'])
        cexp_df.to_csv("cexp_ag{l}.csv".format(l=rhov[i]*10))
        cetraj_df =  pd.DataFrame(ctraj.T, columns = ['x','y','z'])
        cetraj_df.to_csv("ctraj_ag{l}.csv".format(l=rhov[i]*10))
...
#%% Compute angle between CLVs
    dim = len(ctraj[0,:])
    prod12 = np.zeros(dim)
    cos_alpha12 = np.zeros(dim)
    
    prod13 = np.zeros(dim)
    cos_alpha13 = np.zeros(dim)
    
    prod23 = np.zeros(dim)
    cos_alpha23 = np.zeros(dim)
    
    for k in range(dim):
        prod12[k] = np.dot(cvec[:,0,k],cvec[:,1,k])
        cos_alpha12[k] = prod12[k]/(LA.norm(cvec[:,0,k])*LA.norm(cvec[:,1,k]))
        
        prod13[k] = np.dot(cvec[:,0,k],cvec[:,2,k])
        cos_alpha13[k] = prod13[k]/(LA.norm(cvec[:,0,k])*LA.norm(cvec[:,2,k]))
        
        prod23[k] = np.dot(cvec[:,1,k],cvec[:,2,k])
        cos_alpha23[k] = prod23[k]/(LA.norm(cvec[:,1,k])*LA.norm(cvec[:,2,k]))   
#%% Save data: 
    
    # FLE
    # df = pd.DataFrame(fexp)
    # df.to_csv("fexp{k}.csv".format(k=rhov[i]))
    
    # BLE
    # df1 = pd.DataFrame(bexp)
    # df1.to_csv("bexp{k}.csv".format(k=rhov[i]))
    
    # CLE
    # df2 = pd.DataFrame(cexp)
    # df2.to_csv("cexp{k}.csv".format(k=rhov[i]))
    
    #CLVs angle
    
    angles = np.array([cos_alpha12,cos_alpha23,cos_alpha13]).T
    dfangles = pd.DataFrame(angles,columns=["e1e2","e2e3","e1e3"])
    dfangles.to_csv("angles_ag{k}.csv".format(k=rhov[i]))
    
    # traj
    # df3 =  pd.DataFrame(ctraj)
